...
# Tensor Renormalization Groupe (TRG), Higher-Order TRG (HOTRG)  and Bond-weighted TRG (BTRG) for square lattice Ising
import numpy as np
...
def Calculate_TRG(D=4,n=1,kp=0.5,T=2.0,Tmin=1.5,Tmax=3.0,Tstep=0.1,HOTRG_flag=False,Step_flag=False,Append_flag=False):
    ...
    if Step_flag:
        ...
        for T in np.arange(Tmin,Tmax,Tstep):
            ...
            if HOTRG_flag:
                free_energy_density = HOTRG.TRG_Square_Ising(T,D,kp,TRG_step)
            else:
                free_energy_density = TRG.TRG_Square_Ising(T,D,kp,TRG_step)
...
def main():
    ...
    args = parse_args()
    Calculate_TRG(args.D,args.n,args.kp,args.T,args.Tmin,args.Tmax,args.Tstep,args.HOTRG,args.step,args.append)    
